...
The `workers/` directory contains the background task processors that handle the heavy lifting of the Kea research pipeline. These workers decouple long-running cognitive processes from the synchronous API layer, ensuring high throughput and resilience.

## ✨ Features

- **Asynchronous Execution**: Workers process jobs from a priority queue, allowing the system to handle hundreds of concurrent research tasks.
...
```mermaid
graph TD
    Queue[Job Queue / Redis] --> RW[Research Worker]
    Queue --> SLW[Shadow Lab Worker]
    Queue --> SW[Synthesis Worker]
...
# Shadow Lab Worker
python -m workers.shadow_lab_worker

# Synthesis Worker
python -m workers.synthesis_worker
```
