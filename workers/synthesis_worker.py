...
class SynthesisWorker:
    ...
    async def _get_next_task(self) -> dict | None:
        """Claim the next pending synthesis task from execution_batches."""
        try:
            from shared.dispatcher import get_dispatcher

            dispatcher = get_dispatcher()
            return await dispatcher.claim_next_task(batch_type="synthesis")
        except Exception as e:
            logger.warning(f"Synthesis task poll failed: {e}")
            return None
