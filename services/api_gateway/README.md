...
- **`routes/`**: contains the implementation of API endpoints. These modules primarily serve as "orchestrators" that validate inputs and delegate work to internal clients.
...
- **Backend**: Postgres Unlogged Table for global state, providing high throughput without the overhead of WAL logs.
- **Fail-Open Logic**: If the database becomes unavailable, the Gateway defaults to allowing requests to prioritize system availability over strict enforcement.
