                ...
                if subtask.preferred_tool:
                    ...
                    if server_name:
                        try:
                            ...
                            # Basic argument construction (naive for now, improved later)
                            # We assume query/code/url based on tool type
                            args = {}
                            if tool_name in ["web_search", "news_search", "human_search"]:
                                args = {"query": subtask.query}
                            elif tool_name in ["execute_code", "run_python"]:
                                args = {"code": f"# Solver for: {subtask.query}\n# Code generated by Swarm"}
                            elif tool_name in ["get_idx_tickers"]:
                                args = {"index_name": "JKSE"} # Default safe arg
                            else:
                                # Generic fallback
                                args = {"query": subtask.query}
