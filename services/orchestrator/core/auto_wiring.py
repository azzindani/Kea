...
class AutoWirer:
    ...
    async def wire_inputs(self, tool_name: str, explicit_inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if still_missing and available_artifacts:
            logger.info(f"🧠 Invoking LLM Auto-Wiring for {len(still_missing)} args: {still_missing}")
            try:
                ...
                for arg, mapping in llm_wiring.items():
                    ...
                    if ref:
                        ...
                        candidate = next((c for c in available_artifacts if c.source_ref == ref), None)
                        if candidate:
            ...
            if isinstance(obj, dict):
                for k, v in obj.items():
                    if isinstance(k, str): # Only string keys
                        ...
                        _recurse(v, k, new_ref, depth + 1) # Note: passing 'k' as artifact_key for heuristic matching
    ...
    async def _ask_llm_for_mapping(
        ...
        for c in candidates[:50]: # First 50 candidates only
            if c.source_ref in seen_refs: continue
            ...
            candidate_summary.append(f"- Ref: {c.source_ref} | Key: {c.artifact_key} | Type: {c.data_type} | Val: {val_str}")
        ...
        messages = [
            LLMMessage(
                role=LLMRole.SYSTEM,
                content="You are a data mapping assistant. You connect data pipelines intelligently."
            ),
            LLMMessage(
                role=LLMRole.USER,
                content=prompt
