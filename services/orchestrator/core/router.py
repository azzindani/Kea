...
class IntentionRouter:
    ...
    def __init__(self):
        self.system_prompt = """You are a research query router. Classify queries into paths:
...
Respond with ONLY the letter: A, B, C, or D"""
    ...
    def _heuristic_route(self, query: str, context: dict | None) -> PathType | None:
        ...
        if any(word in query_lower for word in ["compare", "meta-analysis", "across", "review of studies"]):
            return "C"
        ...
        if context and context.get("prior_research"):
            if any(word in query_lower for word in ["follow up", "continue", "more about", "expand on"]):
                return "A"
