...
class Dispatcher:
    ...
    async def get_batch_status(self, batch_id: str) -> BatchStatus:
        ...
        async with pool.acquire() as conn:
            batch_status = await conn.fetchval(
                "SELECT status FROM execution_batches WHERE batch_id = $1", batch_id
            )
    ...
    async def claim_next_task(self, batch_type: str | None = None) -> dict | None:
        """
        Atomically claim the next pending micro_task from execution_batches.

        Uses SELECT … FOR UPDATE SKIP LOCKED so multiple workers compete safely.
        Returns a plain dict with task fields, or None if the queue is empty.
        batch_type filters by the 'task_type' column when provided.
        """
        await self.ensure_schema()
        from shared.database.connection import get_database_pool as get_db_pool
        ...
        if batch_type:
            row = await pool.fetchrow(
                ...
                WHERE task_id = (
                    SELECT mt.task_id
                    FROM micro_tasks mt
                    JOIN execution_batches eb ON eb.batch_id = mt.batch_id
                    WHERE mt.status = 'pending'
                      AND (eb.status = 'pending' OR eb.status = 'processing')
                      AND ($1::text IS NULL OR mt.task_type = $1)
                      AND (mt.locked_until IS NULL OR mt.locked_until < NOW())
