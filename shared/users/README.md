...
The `shared/users` module provides the core identity, profile, and API key management system for the Kea v0.4.0 architecture. It enables secure multi-user system sessions and delegated authorization for service-to-service communication.

## ✨ Features

- **Role-Based Access Control (RBAC)**: Supports multiple user roles (Admin, Automaton, Standard) to gate system capabilities.
...
- **Profile Persistence**: Manages persistent user preferences, job history, and organizational tags.
...
### 2. Secure API Keys
API keys are hashed with HMAC-SHA256 before storage. The `ApiKeyManager` supports expiration, usage quotas, and semantic scoping, allowing a key to be restricted to a specific system project or department (e.g., "Finance ONLY").
