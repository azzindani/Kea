...
async def test_geopy_real_simulation():
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            if not res.isError:
                print(f" [PASS] Coords: {res.content[0].text}")
            ...
            if not res.isError:
                print(f" [PASS] Address: {res.content[0].text}")
            ...
            if not res.isError:
                print(f" [PASS] Distance: {res.content[0].text} km")
            ...
            if not res.isError:
                 print(f" [PASS] Route: {res.content[0].text}")
