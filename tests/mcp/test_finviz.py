...
async def test_finviz_real_simulation():
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            if not res.isError:
                print(f" \033[92m[PASS]\033[0m Top Gainers: {res.content[0].text[:100]}...")
            else:
            ...
            if not res.isError:
                print(f" \033[92m[PASS]\033[0m Description: {res.content[0].text[:100]}...")
            else:
            ...
            if not res.isError:
                print(f" \033[92m[PASS]\033[0m News: {res.content[0].text[:100]}...")
            else:
                print(f" \033[91m[FAIL]\033[0m {res.content[0].text}")
