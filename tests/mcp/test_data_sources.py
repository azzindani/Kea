...
async def test_data_sources_real_simulation():
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            if not res.isError:
                print(f" [PASS] YFinance Data: {res.content[0].text[:100]}...")
            else:
            ...
            if not res.isError:
                print(f" [PASS] FRED Data: {res.content[0].text[:100]}...")
            else:
            ...
            if not res.isError:
                print(f" [PASS] World Bank Data: {res.content[0].text[:100]}...")
            else:
                print(f" [FAIL] {res.content[0].text}")
