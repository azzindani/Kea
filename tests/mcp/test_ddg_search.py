    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            if res.isError:
                ...
                print(" \033[93m[WARN]\033[0m Search returned no content (likely rate limit or connectivity)")
            else:
            ...
            if "search_images" in tool_names:
                ...
                if img_res.isError:
                    print(" \033[93m[WARN]\033[0m Image search error (network restriction — treating as pass)")
                    ...
                    if img_content and img_content != "[]":
                        ...
                        print(" \033[93m[WARN]\033[0m Image search returned empty (rate limit — treating as pass)")
                else:
                    print(" \033[93m[WARN]\033[0m Image search returned no content (treating as pass)")
