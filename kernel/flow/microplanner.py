...
OUTPUT ONLY JSON. No prose."""
        ...
        try:
            ...
            if not isinstance(data, dict):
                logger.warning(f"  Microplanner LLM returned non-dict JSON: {type(data)} -> {data}")
                return None
            ...
            if action == "continue":
                ...
                for step in new_steps:
                    if not isinstance(step, dict):
                         logger.warning(f"  Microplanner skipped invalid step (not dict): {step}")
                         continue
            ...
            logger.warning(f"  Microplanner LLM failed: {e}")
