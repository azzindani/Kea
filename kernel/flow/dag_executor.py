...
from kernel.core.assembler import (
    ArtifactStore,
...
class DAGExecutor:
    ...
    def _get_ready_nodes(self) -> list[WorkflowNode]:
        ...
        for node_id, node in self._nodes.items():
            ...
            if deps_met and not deps_failed:
                ...
                logger.warning(f"  Node {node_id} skipped: upstream dependency failed")
    ...
    async def _execute_node(self, node: WorkflowNode) -> None:
        ...
        async with self._semaphore:
            try:
                ...
                if cached_result:
                    logger.info(f"  Reusing cached artifact for {node.id} (skipping execution)")
    ...
    async def _execute_loop(self, node: WorkflowNode) -> NodeResult:
        ...
        loop_data = None
        if node.loop_over:
            from kernel.core.assembler import _resolve_jsonpath
...
def _evaluate_condition(condition: str, store: ArtifactStore) -> bool:
    ...
    """
    from kernel.core.assembler import PLACEHOLDER_PATTERN, _resolve_jsonpath
    ...
    for match in matches:
        ...
        if value is not None:
            ...
            logger.warning(f"  Switch condition: unresolved placeholder '{match}'")
            return False
    ...
    try:
        ...
        logger.warning(f"  Switch condition eval failed: '{resolved_condition}'   {e}")
        return False
