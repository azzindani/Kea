...
async def calculate_put_call_parity(arguments: dict) -> ToolResult:
    """
    ...
    """
    try:
        c = arguments['call_price']
        p = arguments['put_price']
        u = arguments['underlying']
        s = arguments['strike']
        r = arguments['interest'] / 100.0 # Standardize to decimal? Mibian takes 0-100 usually?
        ...
        rate_decimal = arguments['interest'] / 100.0
...
async def calculate_moneyness(arguments: dict) -> ToolResult:
...
async def calculate_probability_itm(arguments: dict) -> ToolResult:
    """
    ...
    try:
        ...
        if d <= 0: return dict_to_result({"prob_itm": 0 if u < s else 1}, "Probability ITM")
...
async def calculate_dual_delta(arguments: dict) -> ToolResult:
    # Alias for Prob ITM
    return await calculate_probability_itm(arguments)
