...
async def read_vfs_resource(path: str) -> str:
    """Read a file from the VFS as a resource."""
    # This enables "Reading" via the MCP resource protocol
    content = await fs_read(f"/{path}")
    return content
...
async def fs_ls(path: str = "/") -> str:
    ...
    Args:
        path: The directory path (e.g. "/projects" or "/")
    """
    if not DATABASE_URL:
        return "Error: DATABASE_URL not set."
...
async def fs_mkdir(path: str) -> str:
    ...
    Args:
        path: Full path of the folder (e.g. "/projects/new_project")
    """
    if not DATABASE_URL:
        return "Error: DATABASE_URL not set."
...
async def fs_write(path: str, content: str, description: str = "") -> str:
    ...
    Args:
        path: Full path (e.g. "/notes/thought.txt")
